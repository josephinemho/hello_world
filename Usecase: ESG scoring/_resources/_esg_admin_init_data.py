...
# MAGIC %md
# MAGIC ### Initialisation resources
# MAGIC Don't run this notebook during the demos, it's used to fetch the initial dataset only
...
storage = '/tmp/demo/esg'

dbutils.fs.mkdirs(storage+'/pdf')
dbutils.fs.mkdirs(storage+'/logo')
for index, row in esg_df.iterrows():
  r = requests.get(row['url'])
  pdf_bytes = BytesIO(r.content)
  with open(f"/dbfs{storage}/pdf/{row['ticker']}.pdf", 'wb') as f:
    f.write(pdf_bytes.getbuffer())
  r = requests.get(row['logo'])
  pdf_bytes = BytesIO(r.content)
  extension = row['logo'].rsplit('.', 1)[-1].lower()
  with open(f"/dbfs{storage}/logo/{row['ticker']}.{extension}", 'wb') as f:
    f.write(pdf_bytes.getbuffer())
